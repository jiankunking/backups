 ...
 * ConcurrentHashMap是支持完全并发检索和高并发更新的哈希表。
 ...
 * 该类遵循与Hashtable同样的方法规范，并提供与之相对应的方法。
 * However, even though all operations arethread-safe, retrieval operations do <em>not</em> entail locking,and there is <em>not</em> any support for locking the entire table in a way that prevents all access.
 * 即使所有的操作都是线程安全的，但检索操作是不需要加锁的，并且不支持任何一种方式通过锁定整个表以阻止所有的访问。
 ...
 * 这个类与Hashtable在程序上完全可以互操作，这依赖于它的线程安全，而不是在它的同步细节上。
 * <p>
 * <p>
 ...
 * 检索操作(包括get)通常不会阻塞，因此可能与更新操作(包括put和remove)同时进行。
 ...
 * 更正式,给定键的更新操作与该键的任何（非空）检索操作所显示的值具有happens-before关系。
 ...
 * 对于像putAll和clear这样的聚合操作，并发检索可能反映的是一部分数据的插入或删除。
 ...
 * 类似地，Iterators，SplIterators和Enumerations返回反映迭代器/枚举创建时或之后的某个时间点的哈希表的状态的元素。
 * They do <em>not</em> throw {@link java.util.ConcurrentModificationException ConcurrentModificationException}.
 * terators、 Spliterators 、Enumerations 迭代器不会抛出java.util.ConcurrentModificationException异常。
 ...
 * 请记住，聚合状态方法的结果，包括size、isEmpty和containsValue，只有当map没有在其他线程中进行并发更新时才有用。
 * Otherwise the results of these methods reflect transient states that may be adequate for monitoring or estimation purposes, but not for program control.
 * 否则，这些方法的结果反映了瞬态状态这可能足以用于监视或评估目的，但不是程序控制。
 * <p>
 * <p>
 * <p>The table is dynamically expanded when there are too many collisions (i.e., keys that have distinct hash codes but fall into the same slot modulo the table size), with the expected average effect of maintaining roughly two bins per mapping (corresponding to a 0.75 load factor threshold for resizing).
 * 当存在太多的冲突（即，具有不同的哈希码但是以表的大小为模数落入相同的位置）时，该表被动态扩展，并且每个映射保持大致两个bin的预期平均效果（对应于0.75负载调整大小的因子阈值）。
 ...
 * 另一个构造函数的可选{@code loadFactor}参数提供了另外的方法，通过指定在计算给定数量的元素时要分配的空间量时使用的表密度来定制初始表容量。
 ...
 * interfaces.
 * 该类及其视图和迭代器实现Map和 terator接口的所有可选方法。
 * <p>
 * <p>
 * <p>Like {@link Hashtable} but unlike {@link HashMap}, this class
 * does <em>not</em> allow {@code null} to be used as a key or value.
 * 与Hashtable相似，但与 HashMap不同，这个类不允许使用null作为键或值。
 * <p>
 * <p>
 ...
 * ConcurrentHashMaps支持一系列顺序和并行批量操作，与大多数Stream方法不同，即使是由其他线程同时更新的映射，也可以安全的使用它们。
 * <p>
 * <p>
 * for example, when
 ...
 * 有三种操作，每种具有四种形式，接受Keys, Values, Entries, and (Key, Value) 参数和/或返回值的函数。
 * <p>
 * Because the elements of a ConcurrentHashMap
 * are not ordered in any particular way, and may be processed in
 * different orders in different parallel executions, the correctness
 * of supplied functions should not depend on any ordering, or on any
 * other objects or values that may transiently change while
 * computation is in progress; and except for forEach actions, should
 * ideally be side-effect-free.
 * 由于ConcurrentHashMap的元素不以任何特定的方式排序，并且可能会在不同的并行执行中以不同的顺序进行处理，
 * 因此提供的函数的正确性不应该依赖于任何排序，或者任何其他计算进行中可能瞬时变化的对象或值,
 * 除了forEach操作,理想情况下没有不良影响。
 * <p>
 * Bulk operations on {@link java.util.Map.Entry}
 ...
 * return type.)</li>
 * Plain reductions（由于没有相应的返回类型，因此（key，value）函数参数没有这种方法的形式）
 * <p>
 * <p>
 ...
 * 设置参数1，通过将其划分为足够多的子任务，从而充分利用fork common pool()的所有并行计算,从而实现最大的并行性。
 * <p>
 * Normally, you would initially choose one of these
 * extreme values, and then measure performance of using in-between
 * values that trade off overhead versus throughput.
 * 通常，您将首先选择其中一个极端值，然后测量使用中间值之间的性能，从而权衡开销与吞吐量之间的关系。
 * <p>
 * <p>
 * <p>The concurrency properties of bulk operations follow
 * from those of ConcurrentHashMap: Any non-null result returned
 * from {@code get(key)} and related access methods bears a
 * happens-before relation with the associated insertion or update.
 * 批量操作的并发属性遵循ConcurrentHashMap的并发属性：{@code get（key）}与相关的访问方法所返回的任何非空结果，与相关的插入或更新操作存在happens-before的关系。
 * <p>
 * <p>
 * The result of any bulk operation reflects the composition of these per-element relations (but is not
 * necessarily atomic with respect to the map as a whole unless it is somehow known to be quiescent).
 * 任何批量操作的结果都反映了这些每个元素关系的组成（如果不是以某种方式被认为是静止不变的，那么它相对于整个map就不一定是原子的）。
 * <p>
 * <p>
 * Conversely, because keys
 * and values in the map are never null, null serves as a reliable
 * atomic indicator of the current lack of any result.
 * 相反，由于映射中的键和值从不为空，所以null作为目前缺乏任何结果的可靠原子指标。
 * To maintain this property, null serves as an implicit basis for
 ...
 * 作为参数提供的搜索和转换函数应该类似地返回null以表示缺少任何结果（在这种情况下不被使用）。
 ...
 * 例如，当找到最大值时，它们可能是有用的。
 * Note that "plain" Entry
 * arguments can be supplied using {@code new AbstractMap.SimpleEntry(k,v)}.
 * <p>
 * <p>Bulk operations may complete abruptly, throwing an
 * exception encountered in the application of a supplied function.
 * 批量操作可能突然完成，抛出在应用程序中遇到的异常。
 * Bear in mind when handling such exceptions that other
...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
     ...
     * excessive memory contention.  The value should be at least DEFAULT_CAPACITY.
     * <p>
     * 每个转移步骤的最小重发次数。范围被细分为允许多个resizer线程。
     * 该值作为下限，以避免调整程序遇到过多的内存争用。
     ...
     * <p>
     * 该类实现了Map.Entry接口，本质是就是一个映射(键值对)
     */
    static class Node<K, V> implements Map.Entry<K, V> {
        final int hash; //用来定位数组索引位置
        final K key;
        volatile V val;
        volatile Node<K, V> next;//链表的下一个node
     ...
     * <p>
     * <p>
     * map构造函数中入参最大值为536870911（2^29 -1）+268435455（2^29 -1 右移1位）+1
     * =805306367=101111111111111111111111111111（一共31位）
     * tableSizeFor方法会对n右移1+2+4+8+16=31次
     * eg1：假设入参c=805306367
     * 则 n=c-1=805306366=101111111111111111111111111110
     * n >>> 1=010111111111111111111111111111
     * 1、n |= n >>> 1;
     * 101111111111111111111111111110
     * 010111111111111111111111111111
     * --------------------
     * 111111111111111111111111111111+1=MAXIMUM_CAPACITY
     * <p>
     * eg2：假设入参c=173
     * 则 n=c-1=172=10101100
     * n >>> 1=01010110
     * 1、n |= n >>> 1;
     * 10101100
     * 01010110
     ...
     * 255+1=256
     * <p>
     * eg3：假设入参c=536870912=100000000000000000000000000000
     * 1、n |= n >>> 1;
     * n >>> 1=010000000000000000000000000000
     * 100000000000000000000000000000
     * 010000000000000000000000000000
     * -------------------------------------
     * 110000000000000000000000000000=n
     * 2、n |= n >>> 2;
     * n >>> 2=001100000000000000000000000000
     * 110000000000000000000000000000
     * 001100000000000000000000000000
     * -------------------------------------
     * 111100000000000000000000000000=n
     * 3、n |= n >>> 4;
     * n >>> 4=000011110000000000000000000000
     * 111100000000000000000000000000
     * 000011110000000000000000000000
     * ---------------------------------------
     * 111111110000000000000000000000=n
     * 4、n |= n >>> 8;
     * n >>> 8=000000001111111100000000000000
     * 111111110000000000000000000000
     * 000000001111111100000000000000
     * ----------------------------------
     * 111111111111111100000000000000=n
     * 5、n |= n >>> 16;
     * n >>> 16=000000000000000011111111111111
     * 111111111111111100000000000000
     * 000000000000000011111111111111
     * ---------------------------------
     * 111111111111111111111111111111=n=1073741823
     * <p>
     * n+1=1073741824=2^30
     * <p>
     * tableSizeFor会将所有位从低至高置成连续的1，最后的 n+1 将保证 capacity 大小为2的整数次幂。
     * <p>
     * c < 1 ：计算出的值将为1；
     */
    private static final int tableSizeFor(int c) {//找到大于等于c的最小的2的幂次方
        int n = c - 1;
        //n >>> 1与n 进行或 运算
        n |= n >>> 1;
        n |= n >>> 2;
     ...
     * -1代表正在初始化
     * -N 表示有N-1个线程正在进行扩容操作
     * 否则，当table为null时，保存创建时要使用的初始表大小，或者默认为0。
     ...
     * 扩容索引，表示已经分配给扩容线程的索引位置。主要用来协调多个线程，并发安全地分配扩容任务。
     * 每个进入transfer方法的线程，都会CAS设置transferIndex索引，
     * 获取需要迁移的node区间，从而确保每个node迁移区间，都只有一个线程迁移，多个迁移线程之间，不会互相影响。
     */
    ...
    public ConcurrentHashMap(int initialCapacity) {
        ...
        //‘>>>’：无符号右移，忽略符号位，空位都以0补齐
        //MAXIMUM_CAPACITY >>> 1，MAXIMUM_CAPACITY 右移1位
        //MAXIMUM_CAPACITY >>> 1=2^29
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            Node<K, V> f;
            int n, i, fh;
            if (tab == null || (n = tab.length) == 0)//table为空，则初始化，不退出循环
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {//如果位置i为空，则直接写入数据（CAS保证并发），退出循环
                ...
                V oldVal = null;
                synchronized (f) {  //对当前位置加锁，然后根据不同类型添加
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {//如果是链表，则以链表的形式添加
                            binCount = 1;
                        ...
                        } else if (f instanceof TreeBin) {//如果是红黑树，则以红黑树的形式添加
                            Node<K, V> p;
                            binCount = 2;
                ...
                if (binCount != 0) {//如果之前是链表结构，但是插入数据后，链表长度超过阈值，则将链表转为红黑树
            ...
            for (Node<K, V>[] tab = nextTable; ; ) {
                ...
                for (; ; ) {// CAS算法多和死循环搭配！直到查到或null
                    ...
                    if ((eh = e.hash) == h &&
                        ...
                        return e;
                    if (eh < 0) {
                        // 列表中元素为ForwardingNode<K, V> 每个ForwardingNode<K, V>都有可能会有nextTable
    ...
    static final int resizeStamp(int n) {
        ...
        //eg 5的二进制表示为：101 最左边开始数起连续的0的个数为：29
    ...
    private final void addCount(long x, int check) {
        CounterCell[] as;
        long b, s;
        //写入baseCount失败，则向对应的countCeller中记录该值
    ...
    private final void tryPresize(int size) {
        ...
        while ((sc = sizeCtl) >= 0) {//没有正在初始化或扩容，或者说表还没有被初始化
            ...
            } else if (c <= sc || n >= MAXIMUM_CAPACITY)// 若欲扩容值不大于原阀值，或现有容量>=最值，break
                break;
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        if (nextTab == null) {            // initiating
            try {
                //把新的数组变为原来的两倍，这里的n<<1就是向左移动一位，也就是乘以二。
                @SuppressWarnings("unchecked")
            ...
            nextTable = nextTab;// target tab 此时内容为空
            transferIndex = n;// source tab length
        }
        int nextn = nextTab.length;//扩容后数组的长度
        //初始化ForwardingNode<K, V> 节点fwd
        //fwd节点会把nextTab放入其Node<K, V>[] nextTable属性中
        ForwardingNode<K, V> fwd = new ForwardingNode<>(nextTab);
        boolean advance = true;//控制是否继续扫描未处理的数据
